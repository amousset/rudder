...
prednre secops au serieux et pas juste en surface

du vrai secops

etape d'apres : utiliser ces infos ds la webapp
...
A long debate in the configuration management world is the choice between
pure data-based configuration (YAML), a declarative DSL and a more imperative approach backed by a
...
Python is maybe a part of the explanation of the success of Ansible.
...
This question is about the way the configuration management tool works at the infrastructure level.
Some are more based on cooperation between agents (like CFEngine), some are very
imperative with a central server connecting to the nodes to run the agent (like Puppet).
...
sequentially, at a given interval.

The only real exception to this rule is the `mgmt` tool, which is
event-based. It works on a configuration graph, which is evaluated once at startup, and
kept in memory.
The agent will then listen for events (file changes using `inotify`, etc.) and re-evaluate
the parts of the graph that are impacted by the event.
It uses functional reactive programming @purpleideaMgmtConfigurationLanguage2018 to achieve this,
exposed through a dedicated functional programming language.
...
- Increased general complexity, a bit harder to reason about
- Some resources are harder to implement in this model
- Prone to "internal drift" in case a notification is missed

Additionally to the local reactive graph approach, mgmt also has
some distributed capabilities, allowing for a more event-driven
...
use data structures, a DSL to describe the policies, or a full programming
language.
...
We would need to plugin the lens reader to a file reactor
to reload specific parts
We would need to keep a history to know about changes though.

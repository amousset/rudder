...
class Boot extends Loggable {
  ...
  def boot(): Unit = {
    ...
    // Display a message instead, as it allows to keep open tabs context until we get the new cookie
    // This does not affect security as it is only a redirection anyway and did not change
    // the session itself.
    LiftRules.noCometSessionCmd.default.set(() ⇒ JsRaw("alert('You have been logged off')").cmd)

...
class TechniqueWriter (
  ...
  def techniqueMetadataContent(technique : EditorTechnique, methods : Map[BundleName, GenericMethod], writtenByRudder : Boolean) : PureResult[XmlNode] = {
    ...
    for {
      ...
      agentSpecificSection <- agentSpecific.traverse(_.agentMetadata(technique, methods, writtenByRudder))
    } yield {
  ...
  def writeTechnique(technique : EditorTechnique, methods: Map[BundleName, GenericMethod], modId : ModificationId, committer : EventActor) : IOResult[EditorTechnique] = {
    for {
      ...
      writtenByRudder <- compiler.compileTechnique(technique, targets, methods, cfengineFallbackTechniqueWriter, dscFallbackTechniqueWriter).map(_ => false).catchAll { e =>
      ...
      metadata   <- writeMetadata(technique, methods, writtenByRudder)
  ...
  def writeMetadata(technique : EditorTechnique, methods: Map[BundleName, GenericMethod], writtenByRudder : Boolean) : IOResult[String] = {
    ...
    for {
      content <- techniqueMetadataContent(technique, methods, writtenByRudder).map(n => xmlPrettyPrinter.format(n)).toIO
...
class ClassicTechniqueWriter(basePath : String, parameterTypeService: ParameterTypeService) extends AgentSpecificTechniqueWriter {
  ...
  def agentMetadata ( technique : EditorTechnique, methods : Map[BundleName, GenericMethod], writtenByRudder : Boolean )  : PureResult[NodeSeq] = {

    // Rudderc manages directly na reporting in bundle. so we only need to do rudder reporting only if it was written by Rudder
    val needReporting =  writtenByRudder && needReportingBundle(technique, methods)
